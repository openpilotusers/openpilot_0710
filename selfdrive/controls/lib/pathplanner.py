...
class PathPlanner():
  ...
  def update(self, sm, pm, CP, VM):
    ...
    self.steerRatio = max(sm['liveParameters'].steerRatio, 0.1)
    angle_offset = sm['liveParameters'].angleOffset
    angleOffsetAverage = sm['liveParameters'].angleOffsetAverage
    stiffnessFactor = max(sm['liveParameters'].stiffnessFactor, 0.1)
    ...
    if (not active) or (self.lane_change_run_timer > LANE_CHANGE_TIME_MAX) or (not self.lane_change_enabled):
      self.lane_change_state = LaneChangeState.off
      self.lane_change_direction = LaneChangeDirection.none
    else:
